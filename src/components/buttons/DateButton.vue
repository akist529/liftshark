...
<div class="DateButton">
	<button title="Last Month" @click="$emit('changeDateBack')">
		<img
			alt="Last Month"
			:src="assetspath('./ui/chevron_left.svg')" />
	</button>
	<button class="calendar-btn" title="Change Date" @click="$emit('openCalendar')">
		<img v-if="dateIsToday()"
			:src="assetspath('./ui/today.svg')" />
		<img v-else
			:src="assetspath('./ui/calendar_month.svg')" />
	...
	<button
		title="Next Month"
		@click="$emit('changeDateForward')">
		<img
			alt="Next Month"
			:src="assetspath('./ui/chevron_right.svg')" />
	</button>
</div>
</template>
